...
@task
def zpod_deploy_config_scripts(*, zpod_id: int):
                ...
                if isinstance(config_scripts, list):
                    if config_scripts:
                        for config_script in config_scripts:
                            print(f"Asked to execute config script {config_script}/zpod_deploy.py for zPod {zpod.name}")
                            # Import dynamically the config script module if it exists
                            try:
                                module_name = f"zpodengine.config_scripts.{config_script}.zpod_deploy"
                                module = __import__(module_name, fromlist=['execute_config_script'])

                                print(f"Executing config script {config_script}/zpod_deploy.py for zPod {zpod.name}")
                                module.execute_config_script(zpod_id=zpod_id)
                            except ImportError:
                                print(f"No config script {config_script}/zpod_deploy.py found for zPod {zpod.name}")
                                pass
                    else:
