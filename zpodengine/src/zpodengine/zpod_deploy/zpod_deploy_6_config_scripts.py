                ...
                if isinstance(config_scripts, list):
                    if config_scripts:
                        for config_script in config_scripts:
                            print(
                                f"Asked to execute config script {config_script}/zpod_deploy.py for zPod {zpod.name}"
                            )
                            ...
                            try:
                                ...
                                print(
                                    f"Executing config script {config_script}/zpod_deploy.py for zPod {zpod.name}"
                                )
                                module.execute_config_script(zpod_id=zpod_id)
                            except ImportError:
                                print(
                                    f"No config script {config_script}/zpod_deploy.py found for zPod {zpod.name}"
                                )
                                pass
                    else:
